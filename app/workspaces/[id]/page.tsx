    ...
    <div className="mx-auto max-w-2xl">
      ...
      <section className="mt-6 rounded-surface border border-paper-200 bg-white p-5 shadow-raised">
        ...
        {members.pendingInvites.length > 0 ? (
          <div className="mt-4">
            <p className="text-caption font-medium text-paper-700">Pending invites</p>
            <ul className="mt-2 space-y-2">
              {members.pendingInvites.map((invite) => (
                <li
                  key={invite.id}
                  className="flex items-center justify-between rounded-surface bg-paper-50 px-3 py-2 text-body text-paper-700"
                >
        ...
        {invited ? (
          <div className="mt-4 rounded-surface border border-status-verified-200 bg-status-verified-50 px-3 py-2 text-body text-status-verified-800">
        ...
        {inviteError ? (
          <div className="mt-4 rounded-surface border border-status-failed-200 bg-status-failed-50 px-3 py-2 text-body text-status-failed-700">
            {inviteError}
          </div>
        ) : null}
